#!/usr/bin/env python
"""Report a PHAROS hypothesis-driven pair analysis.

Read ``positive_control_config.used.json`` and the baseline, selected-pair,
evaluation, and additive-interaction tables in a completed run. Compare the
selected candidate and MOA-defined pairs with random controls, and summarize
additive and order-interaction results when available.

Write ``summary.md``, plotted-value and interaction tables, and comparison
figures under ``run_dir/report/`` or the requested output directory. This report
is generated by ``pharos hypothesis-driven pair`` unless reporting is skipped.
"""
