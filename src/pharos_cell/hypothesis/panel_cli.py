...
def _read_table_auto(path: str | Path) -> Any:
    import pandas as pd
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="pharos hypothesis-driven panel",
        description=(
    ...
    required.add_argument(
        "--approved-pairs-file",
        "--pairs-file",
        dest="approved_pairs_file",
        required=True,
        help="TSV/CSV containing the explicit drug-pair panel.",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    from .engine import run_positive_control_2drug_analysis
    ...
    if args.skip_report:
        ...
        from .reports.panel import make_positive_control_panel_report
