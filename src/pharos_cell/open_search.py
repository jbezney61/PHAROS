...
def _load_runtime_dependencies() -> None:
    """Import heavyweight scientific dependencies only when a search starts."""
    ...
    try:
        ...
        raise RuntimeError(
            "The open-search runtime dependencies are not installed. Install PHAROS "
            "into the documented STATE environment before running a search."
        ) from exc
    torch = torch_module
    yaml = yaml_module
...
def main(argv: Optional[List[str]] = None) -> None:
    """Run the open-search workflow with optional command-line arguments."""
    ...
    if args.skip_report:
        ...
        from .reports.search import main as report_main
