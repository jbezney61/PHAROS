...
All supported workflows are available through the `pharos` command. Install
PHAROS into a Python 3.11 environment from the repository root, then inspect
...
a normal fibroblast state, cancer cells with high EMT toward a state with low EMT, or immune-resistant
