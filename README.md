...
PHAROS is a command-line application for predicting drug combinations that
move cells from a user-defined starting state toward a target state. It turns a
...
First choose the biological states that define the proposed conversion. For
example, a study might ask whether predicted perturbations move metastatic
cells toward a primary-tumor state, drug-resistant cells toward a responsive
state, cancer stem cells toward a differentiated state, cancer associated fibroblasts to
a normal fibroblast state, cancer cells with high EMT toward a state with low EMT,
cancer cells toward healthy cells, or immune-resistant
malignant cells toward an immune-responsive state. PHAROS provides a
data-driven ranking of combinations for that specific start-to-target question;
its predictions are hypotheses to validate experimentally.

The typical CLI workflow is:

1. Run `pharos admissibility` checks for the proposed state conversion.
2. Use `pharos hypothesis-driven` to evaluate prespecified drugs or mechanisms,
   or `pharos open-search` for unbiased combination discovery.
3. Review the reports and statistical evaluations generated by the commands.

For a complete copy-and-run example of this workflow, see the
[single positive-control conversion tutorial](tutorials/README.md). It uses a
Zenodo dataset whose DMSO starting state and panobinostat/crizotinib target
state provide a known two-drug positive control.
...
Create the named `PHAROS` environment from the repository's
...
The reported Python version should begin with `3.11`. The environment file is
intentionally a small, portable bootstrap: it installs Python, `pip`, and
[`uv`](https://docs.astral.sh/uv/pip/environments/), while the editable PHAROS
installation below supplies PHAROS's ordinary Python dependencies. PyTorch and
FAISS are selected separately because their correct packages depend on the
operating system, CUDA support, and available hardware.

### 3. Install PyTorch and FAISS

PHAROS's validated accelerated workflow uses PyTorch 2.7.1 with CUDA 12.8 and
a GPU-enabled FAISS installation. On a Linux x86-64 machine with a compatible
NVIDIA driver, run:
...
The PyTorch command and wheel index come from the
[official PyTorch version matrix](https://pytorch.org/get-started/previous-versions/).
FAISS upstream supports GPU packages through Conda and currently limits the
standard `faiss-gpu` package to Linux x86-64; see the
[official FAISS installation guide](https://github.com/facebookresearch/faiss/blob/main/INSTALL.md).
Do not substitute an unofficial `pip` or `uv pip` package for `faiss-gpu`.

For a CPU-only environment on Linux or Windows, use these commands instead:
...
conda install --channel pytorch --channel conda-forge faiss-cpu=1.14.2
```

On Apple-silicon macOS, install PyTorch without a custom wheel index, followed
by the same `faiss-cpu` Conda command:
...
FAISS does not currently publish that prebuilt CPU package for Intel macOS.
Intel Mac users must follow FAISS's source-build instructions or use a
supported Linux environment.

CPU FAISS can support small smoke tests by passing `--no-require-gpu` to the
...
`uv` recognizes an activated Conda environment and installs into it. Confirm
the command and compute packages are available:

```bash
pharos --help
pharos --version
python -c "import torch, faiss; print('torch:', torch.__version__); print('CUDA:', torch.cuda.is_available()); print('FAISS GPUs:', faiss.get_num_gpus())"
```

PHAROS expects an AnnData (`.h5ad`) dataset containing the starting and target
cell states. The dataset may be observational and does not need to contain
perturbation measurements. Before running an analysis, follow the
[STATE and model setup](#state-source-and-model-prerequisites) below to install
the validated STATE version and download the required pretrained models.
...
not use STATE's transition-inference commands. STATE's CLI is used only to
generate `X_state` embeddings when they are not already present in the input
dataset.
...
`pharos evaluate pair-recovery --help` for complete option references.
